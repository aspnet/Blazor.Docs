...
| [Scoped](https://docs.microsoft.com/dotnet/api/microsoft.extensions.dependencyinjection.servicedescriptor.scoped) | Blazor doesn't currently have the concept of DI scopes. `Scoped` behaves like `Singleton`. Therefore, prefer `Singleton` and avoid `Scoped`. However, Server-Side Blazor also known as Razor Components, does support the idea of `Scoped`. In this case, the service is scoped to the `Session`. |
